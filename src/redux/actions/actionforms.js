...
export const GET_AFORMS_TABS = 'GET_AFORMS_TABS';
export const GET_AFORMS_SLOTS = 'GET_AFORMS_SLOTS';
// export const GET_CE_CHANNEL_TABS = 'GET_CE_CHANNEL_TABS';
// export const GET_CE_CHANNEL_SLOTS = 'GET_CE_CHANNEL_SLOTS';
export const GET_SIDEBAR_AFTYPES = 'GET_SIDEBAR_AFTYPES';
export const GET_SIDEBAR_AFNAMES = 'GET_SIDEBAR_AFNAMES';
  ...
  switch(slot) {
    ...
    case 'S21':
      import('../../components/borsuk-action-form.js').then((module) => {
        // tu będzie set filter na operacyjne
      });
      break;
    case 'S22':
      import('../../components/borsuk-action-form.js').then((module) => {
        // tu będzie set filter na marketingowe
      });
      break;
    case 'S99':
...
export const getAFormsTabs = (aFormsTabs) => (dispatch) => {
  const aformstabs = aFormsTabs.reduce((obj, option) => {
    obj[option.tabIndex] = option
    return obj
  }, {});

  dispatch({
    type: GET_AFORMS_TABS,
    aformstabs
  });
};

export const getAFormsSlots = (aFormsSlots) => (dispatch) => {
  const aformsslots = aFormsSlots.reduce((obj, option) => {
    obj[option.tabPageId] = option
    return obj
  }, {});

  dispatch({
    type: GET_AFORMS_SLOTS,
    aformsslots
  });
};
...
export const getSidebarAFTypes = (sidebarAFTypes) => (dispatch) => {
  const aftypes = sidebarAFTypes.reduce((obj, option) => {
    obj[option.index] = option
    return obj
  }, {});

  dispatch({
    type: GET_SIDEBAR_AFTYPES,
    aftypes
  });
};

export const getSidebarAFNames = (sidebarAFNames) => (dispatch) => {
  const afnames = sidebarAFNames.reduce((obj, option) => {
    obj[option.subtypeId + option.index] = option
    return obj
  }, {});

  dispatch({
    type: GET_SIDEBAR_AFNAMES,
    afnames
  });
};
